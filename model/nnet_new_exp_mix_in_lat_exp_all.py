...
class DMTEVT_model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # nn.Linear(self.hparams.num_use_moe, num_input_dim)
        # nn.Parameter(
                # torch.randn(self.hparams.num_use_moe, num_input_dim).to(self.device)
            # )
    ...
    def forward(self, x, tau=100.0):
        ...
        if len(x.shape) == 2:
            ...
            # expanded_cond = self.vis(expanded_enc_out.reshape(expanded_enc_out.shape[0], -1))
        ...
        # lat_high_dim = lat_higt_dim_out.mean(dim=1)
        # import pdb; pdb.set_trace()
        lat_high_dim = lat_higt_dim_out.reshape((batch_size*2, -1))
        lat_vis = self.vis(lat_high_dim)
        # lat_vis_out = self.vis(lat_higt_dim_out.reshape((-1, lat_higt_dim_out.shape[-1])))
        # lat_vis_out = lat_vis_out.reshape((batch_size*2, -1, lat_vis.shape[-1]))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.init_exp_bool:
            ...
            kmeans = KMeans(n_clusters=self.hparams.num_use_moe, random_state=0)  # 假设我们希望分成10个簇
            kmeans.fit(data_np)
            ...
            # self.exp = nn.Parameter(cluster_matrix_tensor.t().to(self.device))
            # import pdb; pdb.set_trace()
        ...
        # loss_all = (loss_emb + loss_lat) / 2 + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        loss_all = (loss_emb + loss_lat) / 2 + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
    ...
    def noise_map(self, data, num_exp = 10,  noise_level=0.1):
        ...
        distance_tensor = torch.stack(distance_list, dim=1)
            # noise_data = data + torch.randn_like(data[:, i*exp_feature_num:(i+1)*exp_feature_num])*noise_level*data[:, i*exp_feature_num:(i+1)*exp_feature_num].std(dim=0)            
        # import pdb; pdb.set_trace()
        # noise_data = data + torch.randn_like(data)*noise_level*data.std(dim=0)
